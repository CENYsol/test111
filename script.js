...
/* ===== Works carousel: use provided images ===== */
const images = [
...
/* разбиваем на два ряда: нечётные/чётные для разнообразия */
...
function fillRow(containerId, arr){
  ...
  const dom = [];
  // Нужно больше половины ширины → дублируем набор
  const list = arr.concat(arr);
  list.forEach(src=>{
    const img = document.createElement("img");
    img.src = src;
    img.alt = "work";
    img.className = "tile" + (src.endsWith(".gif") ? " is-gif" : "");
    img.loading = "lazy";
    dom.push(img);
  });
